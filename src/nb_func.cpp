...
static arg_data method_args[2] = {
    { "self", nullptr, nullptr, nullptr, nullptr, false, false },
    { nullptr, nullptr, nullptr, nullptr, nullptr, false, false }
};
...
std::string replace_signature(const char *signature) {
    ...
    replace_all(signature_str, "Optional[None]", "Literal[None]");
...
PyObject *nb_func_get_doc(PyObject *self, void *option) {
                ...
                else if (stack.empty())
                {
                    ...
                    if (*cur == ',')
                    {
                        ...
                        *target = strip(std::string_view(start, cur_copy));
                        if (name != "*" && name != "/") arguments.emplace_back(name, hint, defv);
    ...
    const auto put_args = [&num_nest, expand_type_hint](const auto& docstring, const auto& signature, bool admonition = false) {
        auto has_content = signature.first.size() > 1 ||
                           (signature.first.size() == 1 &&
                            (std::get<0>(signature.first[0]) != "self" || !std::get<1>(signature.first[0]).empty()));
        if (has_content) {
            buf.put('\n');
            buf.put(' ', 4 * num_nest);
            // TODO: split
            ...
            for(size_t i = 0; i < signature.first.size(); ++i ) {
                ...
                if (i == 0 && arg_str == "self" && std::get<1>(signature.first[i]).empty()) continue;
                buf.put(' ', 4 * num_nest);
                if (admonition) {
                    buf.put("* ");
                ...
                if ((i < docstring.args.size() && !docstring.args[i].empty()) || !std::get<2>(signature.first[i]).empty())
                    buf.put(":");
